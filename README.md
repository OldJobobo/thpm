...
The Obsidian Terminal integration accepts an optional `obsidian-terminal.json` xterm.js color object. It must be a regular JSON file no larger than 64 KiB, use only recognized xterm color keys, and contain only `#RRGGBB` values. That authored object supersedes semantic generation. THPM records each vault's prior `terminalOptions.theme` and restores it on disable or uninstall only while the installed value still matches THPM; user modifications are preserved.
