...
An integration appearing in THPM does not by itself establish complete end-to-end certification evidence. The current [integration support register](docs/integration-support.md) records the reference Omarchy environment, each active adapter's product status and default, repeatable certification procedures, remaining real-application evidence, retired cleanup, and maintainer signoff. Established built-ins are labeled Supported and retain their intended defaults; an incomplete evidence row is tracked as an audit backlog rather than automatically downgrading a working adapter. SwayNC remains explicitly Experimental and default-disabled until its loader path is validated. Other deliberate opt-in defaults reflect takeover scope or confirmation requirements, not Experimental status. Cava still requires its separate consent marker.
