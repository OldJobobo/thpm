...
The same control appears as **Restart apps automatically** in the GUI and TUI System sections. The default is `automatic`; THPM never launches an application that was closed. Applications without a safe restart contract, including generic GTK applications and nwg-dock-hyprland, are always reported rather than killed. For Zen Browser, automatic restart is strict and best-effort: THPM requires one verified default-profile process and exact Hyprland windows, requests normal window closes without sending kill signals, waits for the profile lock and process to exit, and relaunches that profile once. Multiple profiles, unsupported launch arguments, changing windows, missing session services, and any other ambiguous state remain notification-only. Zen restart support remains Experimental: private windows and prior Hyprland placement are not guaranteed to survive relaunch. **Apply active theme** in either frontend, and the equivalent `thpm run`, explicitly force restart-capable adapters such as Spotify and verified Zen sessions even when their managed files are already current; ordinary theme hooks retain no-op behavior when nothing changed.
