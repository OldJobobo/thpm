...
Interactive commands use a color-aware progress surface with an animated spinner, a stage-based progress bar, elapsed time, semantic success and failure colors, and a verified completion card. Add `--verbose` (or `-v`) for stage logs, changed paths, and captured command output. Animation is automatically disabled when output is redirected; `NO_COLOR=1` disables ANSI color, and `--json` remains stable and decoration-free for automation.
