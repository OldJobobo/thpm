...
| `steam` | Incomplete | Experimental | disabled | Quattro evidence covers the installed package/hook, generated dark/light fallback, Adwaita-for-Steam 4.4 deployment/import/bundle verification, ordinary no-op, manual upstream reversal, and visible client-chrome and Library rendering. Store web content remains native and is not themed by this integration. Live authored-asset rendering plus user-modification/uninstall certification gates remain pending. |
