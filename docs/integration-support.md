...
This baseline records the host used to audit contracts and the successful parent CI run. It is **not** application certification. Complete certifications are recorded separately for `fzf` in `docs/certifications/fzf-2026-08-28.md` and `zellij` in `docs/certifications/zellij-2026-08-28.md`; every other active integration still lacks a complete repository record containing a real-application version, successful loader observation, and maintainer signoff.
...
| `nautilus-palette` | Incomplete | Experimental | disabled | With recorded Nautilus and nautilus-python versions, prove the user extension loads, both fixture palettes repaint an open Nautilus window through CSS hot reload, extension changes report restart, and displaced files are restored or preserved. |
| `gnome-accent-compat` | Incomplete | Experimental | disabled | In a disposable desktop session, record the prior GNOME accent, prove both fixtures map and hot-reload in libadwaita, then verify disable/uninstall restoration and concurrent user-change preservation. |
...
| `zellij` | Complete | Supported | disabled | Certified on Omarchy 4.0.1 with Zellij 0.45.0: one running session visibly repainted across real `omarchy-theme-set` transitions; parser, no-op, disable, user-edit, uninstall, restoration, restart-reporting, and maintainer-signoff evidence is retained in `docs/certifications/zellij-2026-08-28.md`. |
