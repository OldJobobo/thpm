...
| `zen` | Incomplete | Experimental | disabled | Launch recorded `zen-browser` with a disposable profile and userChrome enabled; prove import and rendered chrome, then certify automatic single-window and multi-window session restoration plus notification-only fallback for ambiguous and private-window sessions. |
