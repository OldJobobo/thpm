...
Enabling the Zed integration through the CLI, TUI, or graphical manager performs the one-time settings operation; `thpm zed setup` exposes the same operation directly. It backs up existing Zed settings once under THPM state and uses a token-aware JSONC edit to select **THPM Current** while preserving comments and keys outside the replaced theme value. Theme hooks never rewrite settings. `thpm zed status` reports source precedence, synchronization, the selected string or light/dark theme, and Omazed command/output availability. Omazed remains an independently owned generated fallback; when no authored source exists, THPM reports that the user may select **Omazed** rather than claiming an automatic switch.
