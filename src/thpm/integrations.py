...
def _steam_installer(paths: Paths) -> Path:
    return paths.home / ".local/share/steam-adwaita/install.py"


def _steam_omarchy_color_theme(paths: Paths) -> Path:
    return (
        _steam_installer(paths).parent
        / "adwaita/colorthemes/omarchy/omarchy.css"
    )


def _validate_steam_css(path: Path) -> None:
    if path.is_symlink() or not path.is_file():
        raise ValueError(f"Steam theme must be a regular file: {path}")
    try:
        content = path.read_text(encoding="utf-8")
    except (OSError, UnicodeError) as exc:
        raise ValueError(f"Steam theme is not readable UTF-8 CSS: {path}") from exc
    if not content.strip():
        raise ValueError(f"Steam theme is empty: {path}")
    if "\x00" in content:
        raise ValueError(f"Steam theme contains a NUL byte: {path}")


def _generated_steam_css(colors: dict[str, str]) -> str:
    accent = colors.get("accent", colors["blue"])
    return f"""/* Generated by THPM from Omarchy's normalized semantic palette.
 * Variable contract matches Adwaita-for-Steam 4.4 color themes.
 */
:root {{
  --adw-color-scheme: {colors['mode']} !important;
  --adw-accent-bg-light: var(--adw-system-accent, {accent}) !important;
  --adw-accent-bg-dark: var(--adw-system-accent, {accent}) !important;

  --adw-destructive-bg-light: {colors['red']} !important;
  --adw-destructive-bg-dark: {colors['red']} !important;
  --adw-destructive-fg: {colors['fg']} !important;
  --adw-success-bg-light: {colors['green']} !important;
  --adw-success-bg-dark: {colors['green']} !important;
  --adw-success-fg: {colors['fg']} !important;
  --adw-warning-bg-light: {colors['yellow']} !important;
  --adw-warning-bg-dark: {colors['yellow']} !important;
  --adw-warning-fg: rgba(0, 0, 0, 0.8) !important;
  --adw-error-bg-light: {colors['red']} !important;
  --adw-error-bg-dark: {colors['red']} !important;
  --adw-error-fg: {colors['fg']} !important;

  --adw-window-bg: {colors['bg']} !important;
  --adw-window-fg: {colors['fg']} !important;
  --adw-view-bg: {colors['dark_bg']} !important;
  --adw-view-fg: {colors['fg']} !important;

  --adw-headerbar-bg: {colors['bg']} !important;
  --adw-headerbar-fg: {colors['fg']} !important;
  --adw-headerbar-backdrop: {colors['dark_bg']} !important;
  --adw-headerbar-shade: rgba(0, 0, 0, 0.36) !important;
  --adw-headerbar-darker-shade: rgba(0, 0, 0, 0.9) !important;
  --adw-sidebar-bg: {colors['bg']} !important;
  --adw-sidebar-fg: {colors['fg']} !important;
  --adw-sidebar-backdrop: {colors['dark_bg']} !important;
  --adw-sidebar-shade: rgba(0, 0, 0, 0.36) !important;
  --adw-secondary-sidebar-bg: {colors['bg']} !important;
  --adw-secondary-sidebar-fg: {colors['fg']} !important;
  --adw-secondary-sidebar-backdrop: {colors['dark_bg']} !important;
  --adw-secondary-sidebar-shade: rgba(0, 0, 0, 0.36) !important;

  --adw-card-bg: rgba(255, 255, 255, 0.08) !important;
  --adw-card-fg: {colors['fg']} !important;
  --adw-card-shade: rgba(0, 0, 0, 0.36) !important;
  --adw-dialog-bg: {colors['lighter_bg']} !important;
  --adw-dialog-fg: {colors['fg']} !important;
  --adw-popover-bg: {colors['lighter_bg']} !important;
  --adw-popover-fg: {colors['fg']} !important;
  --adw-popover-shade: rgba(0, 0, 0, 0.36) !important;
  --adw-thumbnail-bg: {colors['lighter_bg']} !important;
  --adw-thumbnail-fg: {colors['fg']} !important;
  --adw-shade: rgba(0, 0, 0, 0.36) !important;
  --adw-banner: {colors['muted']} !important;
  --adw-user-offline: {colors['muted']} !important;
  --adw-user-online: {accent} !important;
  --adw-user-ingame: {colors['green']} !important;
}}
"""

def _steam_theme_content(paths: Paths) -> tuple[str, str | None]:
    authored = paths.current_theme / "steam.css"
    if authored.exists() or authored.is_symlink():
        _validate_steam_css(authored)
        return authored.read_text(encoding="utf-8"), None
    colors = load_palette(paths.current_theme / "colors.toml")
    return _generated_steam_css(colors), colors["mode"]


def _steam_theme_source(paths: Paths) -> tuple[Path, str | None, bool]:
    content, mode = _steam_theme_content(paths)
    data = content.encode()
    target = paths.steam_theme_file
    changed = not target.is_file() or target.is_symlink() or target.read_bytes() != data
    if changed:
        atomic_text(target, content)
    return target, mode, changed


def _steam_receipt_digest(paths: Paths) -> str | None:
    receipt = paths.steam_theme_receipt
...
def _write_steam_receipt(paths: Paths, digest: str) -> None:
    atomic_text(
        paths.steam_theme_receipt,
        json.dumps({"sha256": digest}, separators=(",", ":")) + "\n",
    )


def _steam_helper_supports_color_theme(installer: Path) -> tuple[bool, str]:
    try:
        completed = subprocess.run(
            [str(installer), "--help"],
        ...
        return False, "steam-adwaita help timed out"
    except OSError as exc:
        return False, f"unable to inspect steam-adwaita: {exc}"
    output = f"{completed.stdout}\n{completed.stderr}"
    if completed.returncode != 0:
        return False, "steam-adwaita help failed"
    if "--color-theme" not in output:
        return False, "steam-adwaita installer with --color-theme support"
    return True, ""


def _steam_install_is_verified(paths: Paths, source: Path) -> bool:
    color_theme = _steam_omarchy_color_theme(paths)
    config_css = paths.home / ".steam/steam/steamui/adwaita/config.css"
    css_dir = paths.home / ".steam/steam/steamui/css"
    try:
        config = config_css.read_text(encoding="utf-8")
        patched = []
        for name in ("library.css", "gamerecording.css", "gamenotes.css"):
            with (css_dir / name).open(encoding="utf-8") as stylesheet:
                patched.append(
                    stylesheet.readline(64).strip()
                    in {"/* Adwaita-for-Steam */", "/*patched*/"}
                )
        return (
            source.is_file()
            and not source.is_symlink()
            and color_theme.is_file()
            and not color_theme.is_symlink()
            and color_theme.read_bytes() == source.read_bytes()
            and config_css.is_file()
            and not config_css.is_symlink()
            and "@import" in config
            and "colorthemes/omarchy/omarchy.css" in config
            and all(patched)
        )
    except (OSError, UnicodeError):
        return False
...
def cleanup_steam_assets(
    ...
    warnings: list[str] = []
    color_theme = _steam_omarchy_color_theme(paths)
    color_theme_owned = False
    if paths.steam_theme_file.is_file() and not paths.steam_theme_file.is_symlink():
        source_data = paths.steam_theme_file.read_bytes()
        color_theme_owned = (
            color_theme.is_file()
            and not color_theme.is_symlink()
            and color_theme.read_bytes() == source_data
            and _steam_receipt_digest(paths) == _digest(source_data)
        )
    for target in (paths.steam_theme_file, paths.steam_theme_receipt):
        if target.is_file() and not target.is_symlink():
            target.unlink()
            changed.append(str(target))
        elif target.exists() or target.is_symlink():
            warnings.append(f"preserved unexpected THPM Steam state path: {target}")
    if color_theme_owned:
        color_theme.unlink()
        changed.append(str(color_theme))
    elif color_theme.exists() or color_theme.is_symlink():
        warnings.append(f"preserved modified or externally owned Steam color theme: {color_theme}")
    if changed:
        warnings.append(
            "Steam remains externally patched; use Adwaita-for-Steam to reapply preferred options or uninstall its skin"
        )
    return changed, warnings
...
def inspect_readiness(
    ...
    if (
        ...
        installer = _steam_installer(paths)
        if installer.is_symlink() or not installer.is_file():
            missing.append(f"regular steam-adwaita installer at {installer}")
        else:
            compatible, detail = _steam_helper_supports_color_theme(installer)
            if not compatible:
                missing.append(detail)
        steam_css = paths.home / ".steam/steam/steamui/css"
        if not steam_css.is_dir():
            missing.append(
                f"initialized Steam client at {steam_css} (launch Steam once and let its update finish)"
            )
        try:
            _steam_theme_content(paths)
...
def apply(
    ...
    *,
    ...
    if plugin_id == "obsidian-terminal":
        ...
        script = _steam_installer(paths)
        if script.is_symlink() or not script.is_file():
            return ApplyResult(
                plugin_id,
                "skipped",
                message="steam-adwaita installer is not installed as a regular file",
            )
        compatible, detail = _steam_helper_supports_color_theme(script)
        if not compatible:
            return ApplyResult(plugin_id, "skipped", message=detail)
        source, mode, source_changed = _steam_theme_source(paths)
        if source_changed:
            changed.append(str(source))
        color_theme = _steam_omarchy_color_theme(paths)
        color_theme_data = source.read_bytes()
        color_theme_changed = (
            color_theme.is_symlink()
            or not color_theme.is_file()
            or color_theme.read_bytes() != color_theme_data
        )
        if color_theme_changed:
            atomic_text(color_theme, source.read_text(encoding="utf-8"))
            changed.append(str(color_theme))
        digest = _digest(color_theme_data)
        if (
            not force_reload
            and not color_theme_changed
            and _steam_receipt_digest(paths) == digest
            and _steam_install_is_verified(paths, source)
        ):
            return _result(plugin_id, changed, [], warnings)
        command = [
            str(script),
        ...
        if mode is not None:
            command.extend(["--color-scheme", mode])
        command.extend(["--extras", "library/hide-whats-new"])
        try:
            completed = subprocess.run(
                command,
                cwd=script.parent,
        ...
        if completed.returncode != 0:
            ...
            raise RuntimeError(f"steam: steam-adwaita failed: {failure_detail}")
        if not _steam_install_is_verified(paths, source):
            detail = (
                completed.stderr.strip()
                or completed.stdout.strip()
                or "installer returned success without deploying the Omarchy color theme"
            )
            raise RuntimeError(
                "steam: steam-adwaita did not deploy the Omarchy color theme: " + detail
            )
        _write_steam_receipt(paths, digest)
        changed.append(str(paths.steam_theme_receipt))
        restart_required: list[str] = []
