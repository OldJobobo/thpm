...
:root {{
  ...
  --adw-system-accent: {accent} !important;
  ...
  --adw-headerbar-bg: {colors['darker_bg']} !important;
  --adw-headerbar-fg: {colors['fg']} !important;
  --adw-headerbar-backdrop: {colors['bg']} !important;
  --adw-sidebar-bg: {colors['darker_bg']} !important;
  --adw-sidebar-fg: {colors['fg']} !important;
  --adw-sidebar-backdrop: {colors['dark_bg']} !important;
  --adw-secondary-sidebar-bg: {colors['dark_bg']} !important;
  --adw-secondary-sidebar-fg: {colors['fg']} !important;
  --adw-card-bg: {colors['lighter_bg']} !important;
...
def _steam_helper_supports_custom_css(installer: Path) -> tuple[bool, str]:
    try:
        completed = subprocess.run(
            [str(installer), "--help"],
    ...
    if "--custom-css" not in output:
        return False, "steam-adwaita installer with --custom-css support"
    return True, ""


def _steam_install_is_verified(paths: Paths, source: Path) -> bool:
    custom_css = paths.home / ".steam/steam/steamui/adwaita/custom.css"
    ...
    try:
        ...
        return (
            source.is_file()
            and not source.is_symlink()
            and custom_css.is_file()
            and not custom_css.is_symlink()
            and custom_css.read_bytes() == source.read_bytes()
            and config_css.is_file()
            and not config_css.is_symlink()
            and "@import" in config
            and "custom.css" in config
        )
    except (OSError, UnicodeError):
        return False
...
def inspect_readiness(
    ...
    if (
        ...
        if installer.is_symlink() or not installer.is_file():
            ...
            compatible, detail = _steam_helper_supports_custom_css(installer)
            if not compatible:
...
def apply(
    ...
    *,
    ...
    if plugin_id == "obsidian-terminal":
        ...
        compatible, detail = _steam_helper_supports_custom_css(script)
        if not compatible:
        ...
        if not force_reload and _steam_receipt_digest(paths) == digest:
            return _result(plugin_id, changed, [], warnings)
        command = [
            str(script),
            "--color-theme",
        ...
        command.extend(["--custom-css", str(source)])
        try:
            completed = subprocess.run(
                command,
                cwd=script.parent,
        ...
        if not _steam_install_is_verified(paths, source):
            detail = (
                completed.stderr.strip()
                or completed.stdout.strip()
                or "installer returned success without deploying matching custom CSS"
            )
            raise RuntimeError(
                "steam: steam-adwaita did not deploy matching custom CSS: " + detail
            )
        ...
        return _result(
            plugin_id,
            changed,
            ["steam-adwaita --custom-css THPM-generated-state"],
            warnings,
            restart_required,
        )
